...
const HomePage = ({ onDonateClick }) => {
  ...
  return (
    <div style={{background:'#0B1A4F', color:'#fff', minHeight:'100vh'}}>
      ...
      {showVideoChoice && (
        <div style={{position:'fixed', inset:0, zIndex:50, display:'flex', alignItems:'center', justifyContent:'center', background:'rgba(0,0,0,0.7)', backdropFilter:'blur(4px)', padding:'0 1rem'}}>
          <div style={{width:'100%', maxWidth:'384px', borderRadius:'2rem', border:'1px solid rgba(255,255,255,0.1)', background:'#101a4d', padding:'2rem', textAlign:'center', boxShadow:'0 25px 60px rgba(0,0,0,0.5)'}}>
            <p style={{marginBottom:'1.5rem', fontSize:'1.125rem', fontWeight:600, color:'#fff'}}>What would you like to do?</p>
            <div style={{display:'flex', flexDirection:'column', gap:'1rem'}}>
              <button
                onClick={() => { setShowVideoChoice(false); navigate('/raffle'); }}
                style={{width:'100%', borderRadius:'100px', border:'1px solid rgba(255,255,255,0.15)', background:'rgba(255,255,255,0.05)', padding:'1.25rem', fontSize:'1rem', fontWeight:700, color:'#fff', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap:'0.5rem'}}
              >
                <PlayCircle size={20} /> Watch the Video First
              </button>
              <button
                onClick={() => { setShowVideoChoice(false); onDonateClick(selectedAmount); }}
                style={{width:'100%', borderRadius:'100px', background:'#e8cc74', padding:'1.25rem', fontSize:'1rem', fontWeight:800, textTransform:'uppercase', letterSpacing:'0.08em', color:'#091031', cursor:'pointer', border:'none', display:'flex', alignItems:'center', justifyContent:'center', gap:'0.5rem'}}
              >
                Donate Now <ArrowRight size={18} />
              </button>
            </div>
            <button onClick={() => setShowVideoChoice(false)} style={{marginTop:'1.25rem', fontSize:'0.875rem', color:'rgba(255,255,255,0.5)', background:'none', border:'none', cursor:'pointer'}}>
              Cancel
            </button>
          </div>
        </div>
