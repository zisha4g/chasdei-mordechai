...
const RafflePage = ({ onDonateClick }) => {
  ...
  useEffect(() => {
    ...
    const init = () => {
      ...
      player.getDuration().then((d) => { if (d > 0) videoDuration = d; });
      ...
      player.on('timeupdate', ({ duration }) => {
        // Grab duration from the event if we didn't get it yet
        if (videoDuration <= 0 && duration > 0) videoDuration = duration;
            ...
            {isTrackableVimeo && !unlocked && (
              <div className="px-8 pt-4 pb-2">
                <div className="mb-1 flex justify-between text-xs text-white/40">
                  ...
                  <span>{Math.round(progress * 100)}% / {Math.round(WATCH_THRESHOLD * 100)}% needed to unlock</span>
                </div>
